...
public:
    Game1() : audio(BUZZER_PIN), player(PLAYER_SPRITE, PLAYER_WIDTH, PLAYER_HEIGHT, 0x0000) {
    ...
    void init() {
        ...
        sleep_ms(1000);
        audio.init();
        screen_.display().fillScreen(Display::BLACK);
    }
    ...
    void start() {
        ...
        for (int i = 0; i < MAX_COINS; i++) {
            silverCoins[i].sprite = new Sprite(SILVER_COIN_SPRITE, 16, 16, 0x0000);
            silverCoins[i].active = false;
            silverCoins[i].velocity = 0;
        }

        for (int i = 0; i < MAX_ASTEROIDS; i++) {
            asteroids[i].sprite = new Sprite(ASTEROID_SPRITE, 8, 16, 0x0000);
    ...
    void render() {
        screen_.display();
        player.draw(screen_.display());
    }
    
private:
    void handleInput() {
        uint16_t touch_x, touch_y;
        if (screen_.readTouch(touch_x, touch_y)) {
            screen_.display().fillRect(old_player_x, old_player_y, PLAYER_WIDTH, PLAYER_HEIGHT, Display::BLACK);
    ...
    void handleCollisions() {
        for (int i = 0; i < MAX_COINS; i++) {
            if (silverCoins[i].active) {
                ...
                screen_.display().fillRect(silverCoins[i].sprite->getX(), old_coin_y, 16, 16, Display::BLACK);
                ...
                if (player.collidesWith(*silverCoins[i].sprite)) {
                    silverCoins[i].active = false;
                    screen_.display().fillRect(silverCoins[i].sprite->getX(), silverCoins[i].sprite->getY(), 16, 16, Display::BLACK);
                    audio.playCoin();
                } else {
                    silverCoins[i].sprite->draw(screen_.display());
                }
            }
        }
        ...
        for (int i = 0; i < MAX_ASTEROIDS; i++) {
            if (asteroids[i].active) {
                ...
                screen_.display().fillRect(asteroids[i].sprite->getX(), old_asteroid_y, 8, 16, Display::BLACK);
                ...
                if (player.collidesWith(*asteroids[i].sprite)) {
                    asteroids[i].active = false;
                    screen_.display().fillRect(asteroids[i].sprite->getX(), asteroids[i].sprite->getY(), 8, 16, Display::BLACK);
                    audio.playHit();
                } else {
                    asteroids[i].sprite->draw(screen_.display());
                }
            }
        }
    }
    ...
    void spawnCoins() {
        ...
        if (current_time - silver_coin_last_spawn_time > COIN_SPAWN_INTERVAL_MS) {
            for (int i = 0; i < MAX_COINS; i++) {
                if (!silverCoins[i].active) {
                    int random_x = (frame_count * 37 + i * 17) % (240 - 16);
    ...
    void spawnAsteroids() {
        ...
        if (current_time - asteroid_last_spawn_time > ASTEROID_SPAWN_INTERVAL_MS) {
            for (int i = 0; i < MAX_ASTEROIDS; i++) {
                if (!asteroids[i].active) {
                    int random_x = (frame_count * 43 + i * 23) % (240 - 8);
